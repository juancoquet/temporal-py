...
them by name, never an implementation. Where wall-clock or randomness is needed, use
``workflow.now`` / ``workflow.random`` / ``workflow.uuid4`` rather than the stdlib equivalents.
"""

from temporalio import workflow

from src.activities.example_plan.contract import EXAMPLE_PLAN_ACTIVITY, ExampleRequest
